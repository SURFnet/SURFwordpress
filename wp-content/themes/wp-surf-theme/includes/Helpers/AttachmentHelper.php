<?php

namespace SURF\Helpers;

use WP_Error;
use WP_User;

/**
 * Attachment helper methods
 * Class AttachmentHelper
 * @package SURF\Helpers
 */
class AttachmentHelper
{

	/**
	 * @param int $imageId
	 * @param string $imageSize
	 * @return string
	 */
	public static function getImageUrl( $imageId = 0, $imageSize = 'large' )
	{
		$image = wp_get_attachment_image_src( $imageId, $imageSize );

		return $image[0] ?? '';
	}

	/**
	 * @param string $url
	 * @return int
	 */
	public static function createAttachmentFromUrl( string $url ): int
	{
		$dir      = wp_upload_dir();
		$file     = basename( parse_url( $url, PHP_URL_PATH ) );
		$filename = $dir['path'] . '/' . $file;
		Helper::putContents( $filename, file_get_contents( $url ) );

		return static::insertAttachment( $filename );
	}

	/**
	 * @param int $postId
	 * @param string $url
	 */
	public static function createFeaturedImageFromUrl( int $postId, string $url )
	{
		$attachmentId = static::createAttachmentFromUrl( $url );
		set_post_thumbnail( $postId, $attachmentId );
	}

	/**
	 * @param string $data
	 * @return int
	 */
	public static function createAttachmentFromBase64( string $data ): int
	{
		$dir      = wp_upload_dir();
		$file     = hash( 'sha256', $data );
		$filename = $dir['path'] . '/' . $file;
		Helper::putContents( $filename, base64_decode( $data ) );

		return static::insertAttachment( $filename );
	}

	/**
	 * @param int $postId
	 * @param string $data
	 */
	public static function createFeaturedImageFromBase64( int $postId, string $data )
	{
		$attachmentId = static::createAttachmentFromBase64( $data );
		set_post_thumbnail( $postId, $attachmentId );
	}

	/**
	 * @return string
	 */
	public static function getBase64Placeholder(): string
	{
		return '';
	}

	/**
	 * @param string $filename
	 * @return int|WP_Error
	 */
	protected static function insertAttachment( string $filename )
	{
		$dir          = wp_upload_dir();
		$mime         = mime_content_type( $filename );
		$attachment   = [
			'guid'           => $dir['url'] . '/' . basename( $filename ),
			'post_mime_type' => $mime,
			'post_title'     => preg_replace( '/\.[^.]+$/', '', basename( $filename ) ),
			'post_content'   => '',
			'post_status'    => 'inherit',
		];
		$attachmentId = wp_insert_attachment( $attachment, $filename );

		require_once ABSPATH . 'wp-admin/includes/image.php';
		$attach_data = wp_generate_attachment_metadata( $attachmentId, $filename );
		wp_update_attachment_metadata( $attachmentId, $attach_data );

		return $attachmentId;
	}

    /**
     * @param WP_User|null $user
     * @return array
     */
    public static function listAllowedExtraMimes(?WP_User $user = null): array
    {
        $list = [];
        if (!static::isAllowedToUpload($user)) {
            return $list;
        }

        $types = get_option('options_surf_media_types') ?: [];
        if (empty($types)) {
            return $list;
        }

        if (!is_array($types)) {
            $types = [(string)$types];
        }

        $mapping = static::getMimeMapping();
        foreach ($types as $file_ext) {
            $file_ext  = strtolower(trim((string) $file_ext));
            $mime_type = $mapping[$file_ext] ?? null;
            if (empty($mime_type)) {
                continue;
            }

            $list[$file_ext] = $mime_type;
        }
        return $list;
    }

    /**
     * @param WP_User|null $user
     * @return bool
     */
    public static function isAllowedToUpload(?WP_User $user = null): bool
    {
        $user = $user ?: wp_get_current_user();
        if (!($user instanceof WP_User)) {
            return false;
        }

        if (!user_can($user, 'edit_theme_settings')) {
            return false;
        }

        return true;
    }

    /**
     * @return string[]
     */
    public static function getMimeMapping(): array
    {
        return [
            'md' => 'text/plain',
            // Add additional file types here (extension => mime type)
        ];
    }

    /**
     * @return array
     */
    public static function listExtraMimesForSelect(): array
    {
        return [
            'md' => _x('.md (Markdown)', 'admin', 'wp-surf-theme'),
            // Add additional file extensions here, with their nice name (for the select options)
        ];
    }

}
