...
/**
 * AbstractTaxonomyArchive
 *
 * @class AbstractTaxonomyArchive
 * @abstract
 */
export default class AbstractTaxonomyArchive {
    /** @private */
    static _instance
    ...
    constructor (taxonomy) {
        if (this.constructor === AbstractTaxonomyArchive) throw new Error('Abstract class can\'t be instantiated.')
        if (taxonomy === undefined) throw new Error('You must set the \'taxonomy\' variable.')
        if (!taxonomy) throw new Error('The \'taxonomy\' variable can\'t be empty')
     ...
     */
    startup () {
        this.archive = document.querySelector(`#archive-${this.taxonomy}`)
        if (!this.archive) {
            return
        }
    ...
    setProperties () {
        ...
        this.term = this.form.dataset.term
        ...
        this.intro = this.archive.querySelector('.archive-page__header')
    ...
    async getPosts () {
        ...
        try {
            await (await ajax(
                window.customData.apiBaseURL + this.taxonomy,
                {
                    term: this.term,
                    paged: this.paged,
                    ...this.params,
                ...
                if (intro !== undefined && this.intro) {
                    this.intro.innerHTML = intro
                }
