@php
	...
	/**
	 * @var PostCollection|Vacancy $vacancies
	 * @var Page|null              $page
	 * @var string                 $categoryName
	 * @var array                  $categoryList
	 * @var string                 $hoursName
	 * @var array                  $hoursList
				...
				<div
					...
					<div class="archive__filter-item">
						<button class="archive__filter-item-title"
						        type="button">{{ __('Functioncategory', 'wp-surf-theme') }}
					...
					<div class="archive__filter-item">
						<button class="archive__filter-item-title" type="button">{{ __('Hours', 'wp-surf-theme') }}
							<x-icon icon="chevron-down" sprite="global"/>
						</button>
						<div class="archive__filter-item-list">
							<div class="top-border-left"></div>
							<div class="top-border-right"></div>
							<x-checkbox-filter :name="$hoursName" :list="$hoursList"/>
						</div>
					</div>
				</div>
			</form>
				...
				<div class="archive__content grid">
					@forelse($vacancies as $vacancy)
						<div @class([$columnSpanClass])>
							@include('vacancy.item', ['vacancy' => $vacancy, 'type' => $postItemType, 'headingTag' => 'h2'])
						</div>
					@empty
						<div class="column span-4-sm span-8-md span-12-lg">
							@include('vacancy.not-found')
						</div>
					@endforelse
